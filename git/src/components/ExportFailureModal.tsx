...
export default function ExportFailureModal({ open, onClose, errorMessage, exportedFolderPath, onOpenFolder, onRetry }: Props) {
  const { t } = useI18n();
      ...
      >
        ...
        <div className="flex justify-center mb-4">
          <div className="relative">
            ...
            />
            {/* Red X badge */}
            <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-[#DC2626] flex items-center justify-center animate-checkmark-pop shadow-sm">
        ...
        {/* Error message */}
        {errorMessage && (
          <p className="text-center text-xs font-mono text-[var(--danger)] mb-5 truncate px-2" title={errorMessage}>
            {errorMessage}
          </p>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onRetry}
            className="text-xs bg-[var(--accent)] hover:bg-[var(--accent-hover)] px-3 py-2 rounded-lg text-white transition-colors flex items-center justify-center gap-1.5"
          >
            <RotateCcw size={13} />
            {t('export.retry')}
          </button>
          <button
            onClick={onClose}
            className="text-xs border border-[var(--border)] hover:border-[var(--text-disabled)] px-3 py-2 rounded-lg text-[var(--text-dark)] transition-colors"
          >
            {t('export.close')}
          </button>
        </div>
